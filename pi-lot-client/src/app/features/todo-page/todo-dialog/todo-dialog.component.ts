		...
		<mat-dialog-content>
			<form [formGroup]="formGroup" class="content">
				...
				<mat-form-field appearance="outline">
					...
					<input matInput [matDatepicker]="picker" formControlName="date" [value]="formGroup.controls.date.value | date:'EEE, DD.MM.YYYY'">
...
export class TodoDialogComponent {
	...
	protected onSubmitButtonClick() {
		...
		const resultCategories: TodoCategory[] = this.categories.value.map(catGrp => {
			return {
				id: catGrp.id.value,
	...
	onCategoryToggle(category: TodoCategory, index: number) {
		if (category.id == undefined) {
			this.snackBar.open(`Category without ID found: ${category.name}`, "Close", {duration: 3000});
			return;
		}
		...
		const ctrl = this.categories.controls.find(ctrl => ctrl.value.id.value === categoryCtrl.value.id.value)
	...
	isSelectedCategory(todoCategory: TodoCategory): boolean {
