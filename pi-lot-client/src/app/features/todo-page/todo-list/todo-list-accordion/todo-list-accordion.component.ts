				...
				<mat-expansion-panel-header>
					<mat-panel-title>
						<mat-checkbox matPrefix class="todo-checkbox" [checked]="todo.done"
									  (click)="onItemSelect(todo)" matTooltip="Done">
						</mat-checkbox>
					...
					<mat-panel-description *ngIf="!(isMobile$ | async)">
						<div class="todo-title-description">
							<p [innerHTML]="highlightSearchText(todo.description, searchText)" [ngClass]="todo.done ? 'done' : ''"></p>
						</div>
						<div class="todo-title-actions">
							<ng-container *ngIf="todo.categories.length > 0">
...
export class TodoListAccordionComponent {
	...
	onDeleteClick(todo: Todo) {
		this.dialog.open<DeleteTodoDialogComponent, Todo, boolean>(DeleteTodoDialogComponent, {
			data: todo
		}).afterClosed()
			.subscribe(result => {
				if (result) {
	...
	onEditClick(todo: Todo) {
		this.dialog.open<TodoDialogComponent, Todo, TodoDialogResult>(TodoDialogComponent, {
			data: todo
		}).afterClosed()
			.subscribe(result => {
				if (result) {
